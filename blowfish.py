...
def driver():
    ...
    j = 0
    for i in range(0,18):
        # extract 8 letters from input string
        # i.e (8 * 4 bits per hex letter = 32 bits)
        slice = key[j:j+8]

        # convert that 8 letter hex block to int
        k = int(slice, 16)

        # XOR it with the P array
        p[i] = p[i]^k

        # Modulo ensures that the key is cycled
        j = (j + 8) % len(key)
